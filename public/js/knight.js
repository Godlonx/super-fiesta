...
export default class Knight extends Piece {
    ...
    checkMovementsRight() {
        if (this._pos%8 < 7 && Math.trunc(this._pos/8) < 7) {
            this._possibleMoves.push(this._pos+(17))
        }
        if (this._pos%8 > 0 && Math.trunc(this._pos/8) < 7) {
            this._possibleMoves.push(this._pos+(15))
        } 
        if (this._pos%8 > 1 && Math.trunc(this._pos/8) < 8) {
            this._possibleMoves.push(this._pos+(6))
        }
        if (this._pos%8 < 6 && Math.trunc(this._pos/8) < 8) {
